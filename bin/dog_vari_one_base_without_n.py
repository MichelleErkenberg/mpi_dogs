...
with open(output_file, "w", newline="") as csvfile:
    ...
    for i in range(alignment.get_alignment_length()):
        ...
        # Filter out 'N's and convert to uppercase
        filtered_column = [base.upper() for base in column if base.upper() != 'N']
        
        # If after filtering there are no bases left, skip this position
        if not filtered_column:
            continue
        
        unique_bases = set(filtered_column)
        
        # Check if there are exactly two unique bases in the filtered column
        if len(unique_bases) == 2:
            # Check if the difference is only one base
            base_counts = {base: filtered_column.count(base) for base in unique_bases}
            if min(base_counts.values()) == 1:
                row = [i+1]  # Position
                for base in column:  # Use the original column to preserve 'N's in the output
                    row.append(base)
