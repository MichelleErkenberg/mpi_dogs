...
#extracts the closely related reference dogs for our mpi dogs (if possible) + alignment
python3 diff/extract_sequences.py "$TREE_DIR/mpi_dogs_pub.ref.nwk" "$OUTDIR/combined_pub.ref.trimmed.aln" "$OUTDIR/related_seq.fasta"
mafft "$OUTDIR/related_seq.fasta" > "$OUTDIR/related_seq.aln"

#creating .csv-files to highlight variations between the dogs
python3 diff/dog_vari.py "$OUTDIR/related_seq.aln sequence_diff.csv"
